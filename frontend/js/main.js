...
document.getElementById("loginForm").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      // Buscar os dados do usuário no Firestore para salvar nome, CPF e email
      if (data.tipoUsuario === "aluno") {
        ...
        if (userDocResponse.ok) {
          ...
          localStorage.setItem("email", data.email); // ✅ email do aluno
        } else {
          // Caso não consiga pegar dados do Firestore, salva pelo menos o email
